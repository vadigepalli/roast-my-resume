...
export default function Home() {
  ...
  const handleFile = async (file: File) => {
    ...
    if (file.type === 'application/pdf') {
      if (!pdfReady || !window.pdfjsLib) {
        setError('PDF parser loading. Try again.');
        return;
      }
      setIsParsing(true);
      try {
        ...
        if (fullText.trim()) setResumeText(fullText.trim());
        else setError('Could not extract text. Try pasting instead.');
      } catch (err) {
        setError('Error parsing PDF. Try pasting instead.');
      } finally {
        setIsParsing(false);
      }
  ...
  const getSeverityColor = (s: string) => {
    ...
    if (s === 'major') return 'border-orange-500/50 bg-orange-500/10';
    if (s === 'minor') return 'border-yellow-500/50 bg-yellow-500/10';
        ...
        {!result ? (
          <div className="space-y-8">
            ...
            <div className={`max-w-2xl mx-auto border-2 border-dashed rounded-2xl p-12 text-center transition-all ${dragActive ? 'border-orange-500 bg-orange-500/10' : 'border-white/20 hover:border-white/40'}`} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
              ...
              <label htmlFor="file-upload" className="cursor-pointer">
                {isParsing ? (
                  <><Loader2 className="w-12 h-12 text-orange-500 mx-auto mb-4 animate-spin" /><p className="text-lg text-white mb-2">Parsing PDF...</p></>
                ) : (
                  <><Upload className="w-12 h-12 text-gray-400 mx-auto mb-4" /><p className="text-lg text-white mb-2">Drop your resume here or <span className="text-orange-500">browse</span></p><p className="text-sm text-gray-500">PDF or TXT files</p></>
                )}
              </label>
            </div>

            <div className="max-w-2xl mx-auto">
              <div className="flex items-center gap-4 mb-4"><div className="flex-1 h-px bg-white/10"></div><span className="text-gray-500 text-sm">or paste your resume</span><div className="flex-1 h-px bg-white/10"></div></div>
          ...
          <div className="space-y-8">
            ...
            <div className="text-center space-y-6">
              <div className="w-32 h-32 rounded-full p-2 score-ring mx-auto" style={{ '--score': result.overallScore } as React.CSSProperties}>
                <div className="w-full h-full rounded-full bg-[#0a0a0a] flex items-center justify-center">
                  <div><div className="text-4xl font-bold text-white">{result.overallScore}</div><div className="text-xs text-gray-400">/ 100</div></div>
                </div>
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                {result.roasts.map((roast, i) => (
                  <div key={i} className={`p-4 rounded-xl border ${getSeverityColor(roast.severity)}`}>
                    <h4 className="font-semibold text-white mb-1">{roast.title}</h4>
                    <p className="text-gray-300 text-sm mb-2">{roast.issue}</p>
                    <div className="bg-black/30 rounded-lg p-3"><p className="text-green-400 text-sm"><strong>Fix:</strong> {roast.fix}</p></div>
                  </div>
                ))}
              </div>
              <div className="space-y-6">
                <div className="roast-card p-6 rounded-xl">
                  <h3 className="text-lg font-bold text-white flex items-center gap-2 mb-4"><CheckCircle className="w-5 h-5 text-green-500" /> What Works</h3>
                  <ul className="space-y-2">{result.strengths.map((s, i) => <li key={i} className="text-gray-300 flex items-start gap-2"><span className="text-green-500">✓</span>{s}</li>)}</ul>
                </div>
                <div className="roast-card p-6 rounded-xl">
                  <h3 className="text-lg font-bold text-white flex items-center gap-2 mb-4"><FileText className="w-5 h-5 text-blue-500" /> ATS Score</h3>
                  <div className="flex items-center gap-4 mb-4"><div className="text-3xl font-bold text-white">{result.atsScore}%</div><div className="flex-1 h-2 bg-white/10 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-orange-500 to-green-500 rounded-full" style={{ width: `${result.atsScore}%` }} /></div></div>
                  {result.atsIssues.length > 0 && <ul className="space-y-1">{result.atsIssues.map((issue, i) => <li key={i} className="text-yellow-400 text-sm">⚠️ {issue}</li>)}</ul>}
                </div>
                {result.missingKeywords.length > 0 && (
                  <div className="roast-card p-6 rounded-xl">
                    <h3 className="text-lg font-bold text-white flex items-center gap-2 mb-4"><Target className="w-5 h-5 text-purple-500" /> Missing Keywords</h3>
                    <div className="flex flex-wrap gap-2">{result.missingKeywords.map((k, i) => <span key={i} className="px-3 py-1 bg-purple-500/20 border border-purple-500/30 rounded-full text-purple-300 text-sm">{k}</span>)}</div>
                  </div>
                )}
              </div>
            </div>
            ...
            {result.rewrittenBullets.length > 0 && (
              <div className="roast-card p-6 rounded-xl">
                <h3 className="text-xl font-bold text-white flex items-center gap-2 mb-6"><TrendingUp className="w-5 h-5 text-green-500" /> Upgraded Bullets</h3>
                <div className="space-y-4">{result.rewrittenBullets.map((b, i) => (
                  <div key={i} className="grid md:grid-cols-2 gap-4">
                    <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-lg"><div className="text-xs text-red-400 mb-2">BEFORE</div><p className="text-gray-300 text-sm">{b.original}</p></div>
                    <div className="p-4 bg-green-500/10 border border-green-500/20 rounded-lg"><div className="text-xs text-green-400 mb-2">AFTER</div><p className="text-gray-300 text-sm">{b.improved}</p></div>
                  </div>
                ))}</div>
              </div>
            )}
